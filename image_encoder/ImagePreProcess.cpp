...
void ImagePreProcess::ImgResize(const cv::Mat& img, cv::Mat& resized_img,const int obj_len) 
{
	...
	if (s >= 1) return;
	Size obj_size = img.rows > img.cols ? Size(obj_len, img.rows*s) : Size(img.cols*s, obj_len);
	resize(img, resized_img, obj_size);
}

void ImagePreProcess::imgRGB_HSVQuantization(const Mat &img_rgb, vector<int> &code)
{
...
void ImagePreProcess::imgRGB_HSVQuantization(const Mat &img_rgb, vector<vector<int> > &code)
{
	if (!img_rgb.data) return;
	if (img_rgb.channels() != 3) return;
	...
	Mat res(img_rgb.size(), CV_8UC1);
	vector<vector<int> > code_tmp(img_rgb.rows, vector<int>(img_rgb.cols, 0));
